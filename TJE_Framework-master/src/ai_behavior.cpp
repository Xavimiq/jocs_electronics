...
void AIBehavior::update(float seconds_elapsed)
{
	...
	else if (state == ATTACK)
	{
		...
		if (!canSeeTarget()) {
			...
			state = SEARCH;
			tiempo = 0.f;
			anim_state.goToState(eEnemyStates::ENEMY_WALK, 0.0f);
		}
	}
	else if (state == FLEE)
	{
...
bool AIBehavior::canSeeTarget()
{
	...
	Vector3 player_pos = target.getTranslation();
	player_pos.y += 20.f;

	Vector3 enemy_pos = model->getTranslation();
	enemy_pos.y += 20.f;

	
	Vector3 toTarget = player_pos - enemy_pos;
	float distance = toTarget.length();
	toTarget = toTarget.normalize();

	Vector3 ray_origin = enemy_pos;
	...
	if (toTarget.dot(enemyFront) > 0.5)
	{
		...
		{
			if (EntityButtonDoor* em = dynamic_cast<EntityButtonDoor*>(e)) {
			...
			else if (EntityMesh* em = dynamic_cast<EntityMesh*>(e)) {
				//if (!ec) continue;
