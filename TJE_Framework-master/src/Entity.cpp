...
EntityPrisoner::EntityPrisoner(std::string name, Vector3 pos, Mesh* mesh, Shader* shader, Texture* texture, Texture* texture_dialogue, EntityPlayer* player) : EntityMesh(nullptr, name, mesh, texture, shader, pos)
{
...
void EntityPrisoner::update(float dt) 
{
	...
	if ((player->model.getTranslation() - model.getTranslation()).length() < 50.f && Input::wasKeyPressed(SDL_SCANCODE_T)) {
		isTalking = !isTalking;
		if (isTalking) World::get_instance()->currentMessageGUI = this->Dialogue;
