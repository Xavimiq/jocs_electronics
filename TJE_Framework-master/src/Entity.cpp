...
void EntityMesh::render()
{
	...
	if (cull && (camera->testSphereInFrustum(sphere_center, sphere_radius) == false ||
		camera->eye.distance(globalmatrix.getTranslation()) > 850)) {
		Entity::render();
		return;
	}
...
void EntityPlayer::update(float dt) {
	...
	if (Input::isKeyPressed(SDL_SCANCODE_W) || Input::isKeyPressed(SDL_SCANCODE_UP)) move_dir = (move_dir + front) * 0.45f;
	if ((Input::isKeyPressed(SDL_SCANCODE_W) && Input::isKeyPressed(SDL_SCANCODE_LSHIFT))) move_dir = (move_dir + front) * 0.65f;
	if (Input::isKeyPressed(SDL_SCANCODE_S) || Input::isKeyPressed(SDL_SCANCODE_DOWN)) move_dir = (move_dir - front) * 0.45f;
	if (Input::isKeyPressed(SDL_SCANCODE_A) || Input::isKeyPressed(SDL_SCANCODE_LEFT)) move_dir = (move_dir + right) * 0.45f;
	if (Input::isKeyPressed(SDL_SCANCODE_D) || Input::isKeyPressed(SDL_SCANCODE_RIGHT)) move_dir = (move_dir - right) * 0.45f;
	...
	if (model.getTranslation().x > 1300 && model.getTranslation().x < 2050 &&
		model.getTranslation().z > -50 && model.getTranslation().z < 150) {
	...
	if (!isOnFloor)
	{
		velocity.y = velocity.y - 2.5f * dt * 13.5f;
	}
	else {
...
void EntityEnemy::render()
{
	...
	mesh->renderAnimated(GL_TRIANGLES, &behavior_c.anim_state.getCurrentSkeleton());
...
void EntityEnemy::update(float elapsed_time)
{
	...
	behavior_c.anim_state.update(elapsed_time);
}
...
EntityPrisoner::EntityPrisoner(std::string name, Vector3 pos, Mesh* mesh, Shader* shader, Texture* texture, Texture* texture_dialogue, EntityPlayer* player, int code) : EntityMesh(nullptr, name, mesh, texture, shader, pos)
{
...
void EntityPrisoner::render()
{
	EntityMesh::render();
	//if (isTalking && (model.getTranslation() - model.getTranslation()).length() < 50.f)
	//{
...
{
	EntityMesh::update(dt);
	if ((player->model.getTranslation() - model.getTranslation()).length() < 50.f && Input::wasKeyPressed(SDL_SCANCODE_T)) {
		isTalking = !isTalking;
		if (isTalking) { World::get_instance()->currentMessageGUI = this->Dialogue; World::audios[1]->Play(); }
		else World::get_instance()->currentMessageGUI = nullptr;
		std::cout << "dialogo seleccionada" << std::endl;
	}
	else if (isTalking && (player->model.getTranslation() - model.getTranslation()).length() > 50.f) {
		isTalking = false;
		World::get_instance()->currentMessageGUI = nullptr;
	} 
...
void EntityButtonDoor::OpenDoor(float dt) {
	...
	if (isPressed) {
		time_to_go -= dt;
		std::string my_time = std::to_string(time_to_go);
		drawText(2, 20, my_time, Vector3(1, 0, 0), 2);
		if (time_to_go <= 0.0f) {
			World::audios[5]->Play();
			isPressed = false;
			World::audios[12]->Pause();
...
void EntityItem::update(float dt) {
	if ((this->player->model.getTranslation() - model.getTranslation()).length() < 50.f && Input::wasKeyPressed(SDL_SCANCODE_T))
	{
		this->model.setTranslation(0.f, -60.f, 0.f);
		isCollected = true;
		std::cout << "item seleccionada" << name << std::endl;
		if (!World::get_instance()->Comprovante) {
			World::audios[10]->Play(); World::get_instance()->Comprovante = true;
		}
		else {
			World::audios[11]->Play();
		}
	}
}