...
public:
		...
		EntityMesh(nullptr, name, mesh, texture, shader, pos) 
	{
		...
		behavior_c.anim_state.addAnimationState("data/Animations/idle.skanim", eEnemyStates::ENEMY_IDLE);
		behavior_c.anim_state.addAnimationState("data/Animations/walk.skanim", eEnemyStates::ENEMY_WALK);

		behavior_c.anim_state.goToState(eEnemyStates::ENEMY_WALK);
	}
...
enum eItems { EMPTY, WINE, BURGUER };
...
public:
	...
	EntityPrisoner(std::string name, Vector3 pos, Mesh* mesh, Shader* shader, Texture* texture, Texture* texture_dialogue, EntityPlayer* player, int codeItem);
...
class EntityItem : public EntityMesh {
...
public:
	...
	EntityItem(std::string name, Vector3 pos, Mesh* mesh, Shader* shader, Texture* texture, EntityPlayer* player, int codeItem) :
		EntityMesh(nullptr, name, mesh, texture, shader, pos) {
		this->player = player;
